...
### What the engine does (v2, path-sensitive)
...
2. **Fuzz-bounded rounding** — rounded/truncating stores are nonlinear;
   instead of going opaque they carry an exact drift bound (½ulp per
   ROUNDED store, 1ulp per truncation). Constraints are only trusted
   with a margin beyond the accumulated fuzz — sound, and honest when
   the margin cannot be established.
...
4. **Boundary obligations, solved per path** (the same condition or
   ...
   - *branch boundaries*: the decision value of each affine condition is
     driven to boundary−ulp / boundary / boundary+ulp by linear equation
     solving on the PICTURE grid — through derived-variable chains and
     accumulators, not just direct inputs;
   - *rounding half-boundaries*: for each affine ROUNDED expression, the
     ...
     Nonlinear forms (variable × variable) fall back to the v1
     producer-inversion heuristic; either way, what cannot be realized
     is reported **UNREALIZED, per path** and flows into the
     certificate's gaps.
5. **Path witnesses** — each path's constraint system is solved outright,
   so coverage is a first-class result; a path whose constraints admit no
   assignment is thereby shown infeasible (dead code), which is reported
   rather than counted against coverage silently.
...
- **Nonlinear forms remain out of reach**: variable × variable products
  (e.g. principal × rate × years) and boundaries that must be pinned
  *through* a rounded store (the fuzz band hides the exact decision
  value) are reported UNREALIZED rather than analyzed. Closing these
  needs an SMT backend with exact rounding semantics — that is the next
  step for this layer, and the LEDGER `WS-FEE > WS-FEE-CAP` boundary is
  the committed test case for it.
...
- Paths containing a non-affine condition cannot have a sound witness;
  their coverage is reported "unknown".
