...
/**
 * The repair prompt: the verifier's evidence handed back to the model.
 * Deterministic by construction (the candidate's own Java from the cache,
 * failing cases in fixed order, evidence truncated at fixed bounds), so
 * the repair request has a stable cache key and replays offline like
 * every other call — the non-negotiable survives the loop.
 */
export function buildRepairPrompt(
  ...
  return (
    `Repair round ${round}: your previous translation FAILED verification against ` +
    `the real legacy binary. Fix the Java so every case below passes, changing as ` +
    `little as possible. Return the complete corrected file as a single ` +
    "```java code block and nothing else.\n\n" +
    `--- the failing translation ---\n\n` +
    "```java\n" +
    java +
    "```\n\n" +
    `--- verifier evidence ---\n\n` +
...
/** Format a failed outcome's evidence: compiler output, or up to three
 *  failing differential cases with the legacy (expected) vs candidate
 *  (actual) values per diverging key. */
function formatEvidence(outcome: CandidateOutcome, caseResults: unknown[] | null): string {
  if (!outcome.compile.ok) {
    return `javac --release 21 failed:\n${outcome.compile.output.split(/\r?\n/).slice(0, 20).join("\n")}`;
  }
  const failing = (caseResults ?? []).filter(
    (r) => (r as { status?: string }).status !== "PASS",
  ) as Record<string, unknown>[];
  const lines: string[] = [];
  for (const c of failing.slice(0, 3)) {
    lines.push(`case ${String(c.id)}: ${String(c.status)}`);
    const stdin = (c as { stdin?: string[] }).stdin;
    if (stdin) lines.push(`  stdin: ${JSON.stringify(stdin)}`);
    const diffs = (c as { diffs?: { field?: string; kind?: string; legacy?: string; modern?: string }[] }).diffs;
    for (const m of diffs ?? []) {
      lines.push(`  ${m.field ?? "?"}: expected (legacy) ${m.legacy ?? "<absent>"}, got ${m.modern ?? "<absent>"} [${m.kind}]`);
    }
    const raw = (c as { raw?: { modernStderr?: string } }).raw;
    if (raw?.modernStderr?.trim()) {
      lines.push(`  candidate stderr: ${raw.modernStderr.trim().split(/\r?\n/).slice(0, 4).join(" | ")}`);
    }
  }
  return lines.join("\n") || "no per-case detail captured";
}
...
export async function runMigrate(opts: {
  ...
  /** Per-candidate diff-case results, kept for repair-prompt evidence. */
  const caseResultsById = new Map<string, unknown[]>();
  /** The extracted (headerless) Java per candidate, kept for repair prompts. */
  const javaById = new Map<string, string>();

  const evaluateCandidate = (id: string, label: string, completion: CachedCompletion): CandidateOutcome => {
    ...
    javaById.set(id, java);
    const emittedClass = extractClassName(java);
    if (emittedClass !== className) {
      console.log(`  note: candidate ${id} emitted class ${emittedClass} (expected ${className}); using it`);
    }
    const header =
    ...
    const candDir = resolve(opts.outDir, `candidate-${id}`);
    ...
    if (!outcome.compile.ok) {
      console.log(`  candidate ${id}: COMPILE FAILED — rejected`);
    ...
    const candConfig: DiffConfig = {
      ...diffConfig,
      modern: { argv: ["java", "-cp", candDir, emittedClass], label: `candidate ${id} (${label})` },
    };
    const { results, summary, verdict } = runDiffCases(candConfig, diffBaseDir, cases);
    outcome.verify = { ...summary, verdict };
    caseResultsById.set(id, results as unknown[]);
    console.log(`  candidate ${id}: compiled; verify ${verdict} (${summary.passed}/${summary.total} passed)`);
    if (verdict === "FAIL") printCaseResults(results.filter((r) => r.status !== "PASS"));

    writeFileSync(
      join(opts.outDir, `candidate-${id}.diff-report.json`),
      JSON.stringify({ candidate: id, verdict, summary, cases: results }, null, 2) + "\n",
    );
    ...
    if (opts.propConfigPath && verdict === "PASS") {
      ...
      console.log(
        `  candidate ${id}: layer A gate ${prop.verdict} ` +
  ...
  for (const cand of CANDIDATES) {
    evaluateCandidate(cand.id, cand.label, completions.get(cand.id)!);
  }

  const isWinner = (o: CandidateOutcome): boolean =>
    o.compile.ok &&
    o.verify?.verdict === "PASS" &&
    (!opts.propConfigPath || o.propVerify?.verdict === "PASS");

  let winner = outcomes.find(isWinner) ?? null;

  // --- 5: bounded generate -> verify -> repair rounds -------------------------
  // When no candidate passes, hand each failure back to the model WITH the
  // verifier's evidence (compiler output or failing differential cases) and
  // re-verify the repair exactly like an original candidate. Every repair
  // request flows through the same replay cache — recorded, hashed, and
  // offline-replayable — and the verifier still decides; the model never
  // grades its own homework. Off by default (--max-repairs).
  const maxRepairs = opts.maxRepairs ?? 0;
  for (let round = 1; !winner && round <= maxRepairs; round++) {
    // Round N repairs the round N-1 generation (repairing an earlier
    // generation again with identical evidence is a cache no-op, not a new
    // attempt). Nearest-miss first: compiled beats not-compiled, then most
    // selection cases passed.
    const parents = outcomes
      .filter((o) => (o.id.match(/-r\d+/g)?.length ?? 0) === round - 1 && !isWinner(o))
      .sort((x, y) => Number(y.compile.ok) - Number(x.compile.ok) || (y.verify?.passed ?? -1) - (x.verify?.passed ?? -1));
    console.log(`\n  repair round ${round} (${parents.length} failed candidate(s)):`);
    for (const parent of parents) {
      const parentSpec = CANDIDATES.find((c) => parent.id.startsWith(c.id));
      const java = javaById.get(parent.id);
      if (!parentSpec || !java) continue;
      const evidence = formatEvidence(parent, caseResultsById.get(parent.id) ?? null);
      const req = {
        model: opts.model,
        system: SYSTEM_PROMPT,
      ...
      try {
        completion = await completeWithCache(opts.cacheDir, req, { offline: opts.offline });
      } catch (e) {
        if (e instanceof MissingCacheEntryError) {
          console.error(`  repair of ${parent.id}: MISSING — ${e.message.split("\n")[0]}`);
          console.error(e.message);
          continue; // offline / no creds: the stub names the repair key; other parents may be cached
        }
        throw e;
      }
      const id = `${parent.id}-r${round}`;
      console.log(
        `  repair ${id} (from ${parent.id}): ${completion.fromCache ? "replayed from cache" : "live call"}` +
          ` [${completion.cacheKey.slice(0, 16)}…]` +
          (completion.fromCache
            ? ""
            : ` — ${completion.usage?.inputTokens ?? "?"} in / ${completion.usage?.outputTokens ?? "?"} out tokens,` +
      ...
      const outcome = evaluateCandidate(id, `${parent.label} repair ${round}`, completion);
      if (isWinner(outcome)) {
        winner = outcome;
        break;
      }
